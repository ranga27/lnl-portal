  ...
  useEffect(() => {
    ...
    stepperItems.forEach((step, i) => {
      if (i + 1 <= props.currentStep) {
        step.classList.add('bg-[#F7B919]', 'text-white');
      } else {
        step.classList.remove('bg-[#F7B919]', 'text-white');
      }
    });
    ...
    stepperItems2.forEach((step, i) => {
      if (i + 1 <= props.currentStep) {
        step.classList.add('bg-white', 'text-[#F7B919]');
      } else {
        step.classList.remove('bg-white', 'text-[#F7B919]');
      }
    });
  }, [props.currentStep]);
  return (
    <div className='w-full mx-auto'>
      <div className='w-full flex flex-row items-center justify-center px-32 py-16'>
